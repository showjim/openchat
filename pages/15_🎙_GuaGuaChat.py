...
chatbot_azure.setup_env("./setting/key.txt", "./setting/config.json")
chatbot_azure.initial_stt()
chatbot_or = ChatRobotOpenRouter()
chatbot_or.setup_env("./setting/key.txt", "./setting/config.json")
...
def create_img_from_siliconflow(prompt: str):
    ...
    payload = {
        ...
        # "black-forest-labs/FLUX.1-dev", #"stabilityai/stable-diffusion-3-5-large", #“Kwai-Kolors/Kolors”
        "prompt": prompt,
...
def create_img_store_dict(index_file_list: List[str]):
    st.session_state.FreeIMGDB = {
        os.path.join("./img", file): base64.b64encode(open(os.path.join("./img", file), 'rb').read()).decode('utf-8')
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("Parameters for Chatbot"):
            ...
            aa_chat_mode = st.selectbox(label="`0. Chat Mode`",
                                        options=["CasualChat", "Meta-prompt", "Translate", "西瓜一家-小南瓜",
                                                 "西瓜一家-小东瓜",
                                                 "西瓜一家-Ana"],
                                        index=0,
                                        on_change=set_reload_mode)
            aa_llm_model = st.selectbox(label="`1. LLM Model`",
                                        options=["deepseek/deepseek-chat-v3-0324", "openai/gpt-4.1",
                ...
                for topic_id, topic_name in topics:
                    ...
                    with col1:
                        if st.button(f"📝 {topic_name}", key=f"topic_{topic_id}"):
                            ...
                            with DatabaseConnection(CHAT_HISTORY_PATH) as c:
                                c.execute(
                                    "SELECT role,type,message,img FROM chats WHERE topic_id = ? ORDER BY created_at",
                                    (topic_id,))
            ...
            if len(options) > 0:
                if st.session_state.FreeChatImgReloadFlag == True:
                    ...
                    with st.spinner('Load Image File'):
                        ...
                        # Save user message to database
                        save_chat_to_db(st.session_state.current_topic_id, "user", "image_url",
    ...
    if st.session_state.current_topic_id:
        ...
        if (prompt := st.chat_input("Type you input here")) or (prompt := speech_txt):
            ...
            st.session_state.FreeChatMessages = chatbot_or.control_msg_history_szie(st.session_state.FreeChatMessages,
                                                                                    max_cnt)
                                ...
                                if deltas is not None:
                                    if deltas.content is not None:
                                        ...
                                        if deltas.tool_calls[0].function.arguments is not None:
                                            cur_func_call["function"]["arguments"] += deltas.tool_calls[
